...
11. **すべての** アクティブな `Project` エントリと、その `# Schedule` / `# Plan` セクション。
...
13. 今日に関係する commitment anchor ポリシー（`Organization` / `Area` / `Project` / `Goal`）：
   ...
   - 直近 `WeeklyNote` に書かれた今週の `WeeklyQuotaCommitment` の粗配分と優先順位
...
5. **今週の quota 粗配分と優先順位が見えているか**。`WeeklyQuotaCommitment` 対象があるのに、今週の `WeeklyNote` に週内の見取り図がまったく書かれていなければ、週次の粗配分不足として扱う。日次では、見えている粗配分の範囲で前倒しはしてよい。
...
6. **Project 本文の直近日程**: `Active` Project の `# Schedule` / `# Plan` に、今日〜数日以内の節目や外部イベント基準の日程感があるもの
7. **抽象 Next / 期限付き Task**: 今日扱う候補のうち、タイトルが抽象動詞やトピック名だけで止まっているもの
...
6. **Project 本文の直近日程**
   - `Project::DueDate` が遠くても、本文上の節目が今日や数日後にあるなら、そこで必要な Task を今日の候補へ繰り上げる。
   - その節目を支える `Next` が見えないなら、Project `# Plan` の確認、Task の具体化、または週次計画側の不足として扱う。

7. **抽象 Next / 期限付き Task**
   - 時間枠に入れる前に、具体動詞へ言い換えるか、1回の作業で終わる単位へ分割する。
   - 言い換えできないなら、その Task はまだ `Next` に弱い。Project `# Plan` に戻す、確認質問を作る、または別の着手点を選ぶ。

### Pre-flight 完了基準

- [ ] 期限超過タスクが0件、または全てに今日の処遇が決定済み
- [ ] 締切超過または直近の `Project::DueDate` を持つ Active Project が0件、または全てに今日の処遇が決定済み
- [ ] **すべての Active Project を確認し**、本文の `# Schedule` / `# Plan` にある直近日程の処遇が決定済み
- [ ] 予定超過タスクが0件、または全てに今日の処遇（実行 or 延期理由記録）が決定済み
- [ ] 古い `Next` / `Await` が0件、または全てに具体的なアクションが決定済み
- [ ] 今日扱う Task に、抽象動詞やトピック名のまま時間枠へ入るものがない
...
- 今日の配分は、**今週の WeeklyNote で決められた粗配分と優先順位**を基準にする。
- 週前半に余力があり、他の約束を壊さないなら、**週後半予定の quota を前倒しして進めてよい**。
- 逆に、その日に想定していた quota を後ろ倒しして週後半へ追いやる場合は、残り可処分時間と締切を見直し、週としてまだ成立するかを確認する。
- もし WeeklyNote 側の配分が曖昧なら、その場で完全最適化を作り直すより、週次レビュー側の不備として明示する。ただし、週の目標達成に資する保守的な前倒しは許容する。
...
0. **約束を先に守る**:
   ...
   - そのうえで、すべての `Active` Project の `# Schedule` / `# Plan` にある直近日程も確認し、本文上の約束を落とさない
   - 直近 WeeklyNote にある quota の粗配分と優先順位を確認する
...
2. **今日の `WeeklyQuotaCommitment` を置く**:
   - WeeklyNote の粗配分と優先順位を踏まえて置く
   - 余力がある日に前倒ししてもよい
   - ただし quota を残日数に丸投げする後ろ倒しはしない
...
7. **抽象 Task 名は時間割り当て前に直す**:
   - `考える` `確認する` `対応する` のようなタイトルをそのまま `09:00-10:00` に置かない
   - 何をする1時間なのか分かるタイトルか、補足本文か、分割後 Task を用意する
...
- **日次計画は週次計画の派生**である。WeeklyNote は rough plan として扱い、前倒しの再配置は許容する。ただし、quota 配分やその日の最低枠を後ろ倒しして再構成する場合は、週としてまだ成立するかを明示的に確認する。
...
- `Project::DueDate` が近くなくても、`Active` Project の `# Schedule` / `# Plan` に今日〜数日以内の節目があるなら、それも約束として扱い、今日の処遇を決める。
...
- [ ] **すべての Active Project を見たうえで**、本文の `# Schedule` / `# Plan` にある直近日程を見落としていないか
- [ ] `DailyFloorCommitment` を置くべき対象を後回しにしていないか
- [ ] 今日の quota の前倒しが、他の約束や週全体の成立性を壊していないか
- [ ] 今日予定していた quota を後ろ倒しした場合、その理由と週内の吸収先が明確か
- [ ] `MaximizeWhenPossible` 対象への追加配分は、約束済みタスクを壊していないか
- [ ] `ScheduledDate` / `DueDate` / `Project::DueDate` を変更した場合、理由を記録したか
- [ ] スケジュールに載せた Task が、見ただけで最初の5分の行動を説明できる名前になっているか

## 避けること

- 週内 quota を「今日は気分じゃないから後で」で根拠なく後ろ倒しする

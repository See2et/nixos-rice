...
## 日次計画における Organization コミットメント

日次計画では、Organization 由来の時間確保を次の順番で扱う。

### 1. `DailyFloorCommitment`

- その日も最低限触りたい対象。
- `MinimumMinutes` を満たす枠を、約束済みタスクを壊さない範囲で先に置く。
- 適切な `Next` がない場合は、無理に枠だけ置かず、Project の粒度見直しや `Next` 整備を優先候補として扱う。
...
- `DailyFloorCommitment` 対象にこのポリシーがある場合、`DueDate` / `ScheduledDate` と quota 配分を守ったあと、余剰時間を優先的に寄せる。
...
0. **約束を先に守る**:
   - `DueDate <= today` と `ScheduledDate <= today` を先に扱う
...
- `ScheduledDate <= today` または `DueDate <= today` の未完了タスクがある場合、それらを**今日のスケジュールに含めるか、意図的に延期するか**を最初に決定する。
...
- スケジュール表は plain text だけでも読める形にする
- `:entry{#...}` は補助参照として使う
...
```md
## 今日のスケジュール
- 09:30-10:30 SuteraVR の秘密鍵管理案を整理する
- 11:00-11:30 JVSL の連絡確認を返す
- 20:30-21:30 Comni.pl Web Clipper 要件整理（今週 quota 枠）
...
- [ ] `DueDate` / `ScheduledDate` を持つタスクを見落としていないか

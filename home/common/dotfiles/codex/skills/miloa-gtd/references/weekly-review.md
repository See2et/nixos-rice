...
3. **quota 未配置**: `WeeklyQuotaCommitment` があるのに、今週どこで時間を積むか決まっていない commitment anchor がないか
...
7. **ポリシー競合**: 同じ commitment anchor に `DailyFloorCommitment` と `WeeklyQuotaCommitment` が同時についていないか
8. **課題未決定のレビュー対象**: 当日レビュー対象なのに、今週解くべき課題が決まっていない Organization がないか
9. **抽象 Task 名**: 今週の締切対応や quota 配分に使う Task が、`考える` `進める` `対応する` のような抽象動詞で止まっていないか

...
Task / Project / Organization を本文中で参照するときは、可能な限り `:entry{#...}` を使う。`:entry{#...}` だけで十分な箇所では名称を繰り返さない。Google Calendar 予定など対応する Entry がないものだけを平文で書く。特に WeeklyNote では、表ではなく Organization ごと / 日ごとの見出しを優先する。
...
3. **quota 未配置**: `WeeklyQuotaCommitment` があるのに、今週どこで時間を積むか決まっていない Organization がないか
4. **DailyFloor の形骸化**: 毎日最低限触るはずなのに、実質的に置けない週になっていないか
5. **繰り返し延期パターン**: 同じタスクが `ScheduledDate` を変更し続けていないか
6. **過積載週**: 可処分時間の合計に対して必要作業時間が明らかに超過していないか
7. **ポリシー競合**: 同じ Organization に `DailyFloorCommitment` と `WeeklyQuotaCommitment` が同時についていないか
8. **課題未決定のレビュー対象**: 当日レビュー対象なのに、今週解くべき課題が決まっていない Organization がないか

...
| 戦略的関心領域 | `Area::StrategicBlock = true` |
...
Project は `Area` に紐づける。Area は継続的な関心領域で終了がなく、Project は Area の中で特定の成果を目指す完了可能な単位である。
...
- `Area::StrategicBlock = true` なのに、有効な `Next` が1件もないArea
- `Area::WeeklyReview = true` なのに、週次レビューで見送られ続けているArea
- Area の Description が古く、現在の重点目標とズレている

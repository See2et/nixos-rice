...
| 毎日最低限確保したい対象 | `DailyFloorCommitment` タグを持つ `Organization` |
| 週あたり必要時間を積みたい対象 | `WeeklyQuotaCommitment` タグを持つ `Organization` |
...
### 推奨 Organization ポリシー構造

Organization 自体はハブとして残し、レビュー対象・時間コミットメントは**追加タグ**として表すのを推奨する。
...
- `TargetHours` を持つ。
- 「1週間あたり n 時間を確保したい対象」を表す。
- 実際の配分は WeeklyNote で決める。日次に先送りしない。

### 無効状態を避けるルール

- 同じ Organization に `DailyFloorCommitment` と `WeeklyQuotaCommitment` を同時に付けない。
- `WeeklyReviewTarget` がない Organization にレビュー曜日だけを持たせない。
- quota の存在だけを作って、WeeklyNote 側に週内配分を書かない状態を放置しない。
...
#### 9. Organization ポリシーの破損

- `WeeklyReviewTarget` があるのに `Day` が欠けている
- 同じ Organization に `DailyFloorCommitment` と `WeeklyQuotaCommitment` が同時に付いている
...
- `WeeklyQuotaCommitment` があるのに、直近 WeeklyNote に週内配分が書かれていない

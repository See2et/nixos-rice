...
13. Organization ポリシーに無効状態がないか。特に、レビュータグなのに曜日が欠ける、時間コミットメントタグが複数付く、など。
...
miloa を変更した場合は、何を変更したか、なぜ変更したかをユーザーに伝える。チャット出力は簡潔に保つが、`Next` / `Await`、`ActivatedDate` / `WaitingSince` / `ScheduledDate` / `Project::DueDate`、Project リンク、Organization ポリシーが保守的に処理されたことを検証できるだけの詳細は含める。

...
4. `Organization::StrategicBlock = true` の戦略Organizationの週次目標を設定する
5. 締切タスクと `ScheduledDate` の週内配置を計画する
6. 異常（締切集中、過積載、戦略枠ゼロ）を検出し調整する
7. 必要なら Task の日付更新方針を決める。**週次レビューのデフォルトは WeeklyNote 記録まで**であり、Task 更新は最小限に留める
8. 週次レビュー結果を `WeeklyNote` エントリとして miloa に記録する

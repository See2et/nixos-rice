...
- `Area`: `Name`, `StrategicBlock`, `WeeklyReview`, `Description`.
...
4. 日次計画では、**戦略Area枠を締切タスクより先に検討する**。戦略Areaとは `Area::StrategicBlock = true` の Area である。詳細は `references/area.md` を参照。
5. 戦略Area枠は、ユーザーが拒否しない限り**毎日確保する**。現実的でない場合は、特定の日に集中して確保するよう提案する。
...
Area の定義、スキーマ、`StrategicBlock` / `WeeklyReview` の運用、Area と Project/Task の関係については、`references/area.md` を読む。
...
2. `Area::WeeklyReview = true` の Area を全てレビューし、週間タイムラインを作成する
3. `Area::StrategicBlock = true` の戦略Areaの週次目標を設定する
...
5. **戦略Area枠を先に検討**し、`StrategicBlock = true` の各Areaから最低1件ずつ選定し時間枠を予約する

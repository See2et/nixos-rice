...
- `WeeklyQuotaCommitment` の時間配分は、日次でゼロから行き当たりばったりに決めず、**WeeklyNote 作成時点で週内の粗配分と優先順位を先に決める**。
...
4. 日次計画では、`Task::DueDate` / `ScheduledDate` / `Project::DueDate` の約束を先に守る。そのうえで WeeklyNote 上の週内配分は**rough plan**として参照し、前倒しは許容するが、後ろ倒しは慎重に扱う。
...
7. `WeeklyQuotaCommitment` は、WeeklyNote の週内粗配分と優先順位を踏まえて置く。**その日の分を前倒しして多めに進めることは許容する**が、後ろ倒しして週後半へ追いやる場合は、週全体の成立性を必ず再確認する。
...
6. `WeeklyQuotaCommitment` を持つ commitment anchor については、**WeeklyNote 作成時点で週内の粗配分と優先順位を決める**
...
5. **約束済みタスクを先に扱う**。`Task::DueDate` / `ScheduledDate` / `Project::DueDate` を確認し、WeeklyNote 上の週内配分は rough plan として参照する
6. **`DailyFloorCommitment` を置く**
7. **WeeklyNote の粗配分を踏まえて、今日進める `WeeklyQuotaCommitment` を置く**
...
- [ ] 当日の `DailyFloorCommitment` と WeeklyNote 上の quota 配分・優先順位を見落としていないか

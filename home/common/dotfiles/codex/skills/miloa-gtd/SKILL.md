...
- `Task`: `Status`, `Bucket`, `Context`, `DueDate`, `ScheduledDate`, `Project`, `EstimatedMinutes`.
...
4. 日次計画では、**戦略プロジェクト枠を締切タスクより先に検討する**。このスキルにおける戦略プロジェクトは、**SuteraVR** と **miloa** に関連する `Project` のみとする。
5. 戦略プロジェクト枠を置ける日には、**SuteraVR または miloa のどちらかから最低1件**、その日に具体的な時間枠を確保する候補を選ぶ。候補があるのに枠を置かない場合は、その理由を明示する。
...
2. 週間タイムラインを作成し、SuteraVR / miloa の週次目標を設定する
3. 締切タスクと `ScheduledDate` の週内配置を計画する
4. 異常（締切集中、過積載、戦略枠ゼロ）を検出し調整する
...
5. **SuteraVR / miloa の戦略プロジェクト枠を先に検討**し、置けるなら先に時間枠を予約する

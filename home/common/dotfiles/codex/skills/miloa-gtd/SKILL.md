...
ユーザーの miloa タスクシステムを扱うときは、このスキルを使用する。目的は構造を増やすことではない。目的は `Next` の信頼性を保ち、`Await` を放置されない待ちリストとして保ち、現実的に完了できる日次計画を作ることである。
...
9. `Done` なのに `Next` / `Await` が残っているなど、Bucket と状態の矛盾。

...
以下が揃うまで、`_create_entry` / `_update_entry` / `_delete_entry` を実行しない。

1. `list_tags` を読み、**現在の実スキーマ**を確認した
2. 関連する既存 `Task` / `Project` / 必要なら既存 `DailyNote` / `WeeklyNote` を検索した
3. いま実行しているモードに必要な reference を読んだ
4. 変更対象と変更理由を短く言語化できる

追加ゲート：

- 週次レビューでは、Google Calendar の週予定取得と `Organization::WeeklyReview = true` の全件確認が終わるまで書き込まない
...
- `Organization`: `Name`, `StrategicBlock`, `WeeklyReview`, `Description`.
  ...
  - `Organization::StrategicBlock`: 日次計画で優先して時間枠を確保する対象かどうか。
  - `Organization::WeeklyReview`: 週次レビューで進捗確認し、今週解くべき課題を1つ決める対象とするか。
...
3. `Organization::WeeklyReview = true` の Organization を全てレビューする
4. 各レビュー対象 Organization について、**今週必ず解くべき課題を1つ**決める。AI は候補提案まで行ってよいが、**最終決定は必ずユーザーから引き出す**
5. `Organization::StrategicBlock = true` の Organization については、上で決めた課題を進めるための時間枠をどう確保するかだけを考える。`StrategicBlock` 自体を週次目標管理に使わない
6. 締切タスクと `ScheduledDate` の週内配置を計画する
7. 異常（締切集中、過積載、戦略枠ゼロ、レビュー対象なのに課題未決定）を検出し調整する
8. 必要なら Task の日付更新方針を決める。**週次レビューのデフォルトは WeeklyNote 記録まで**であり、Task 更新は最小限に留める
...
6. **空き時間ブロックを特定**し、実質稼働可能時間を計算
7. **`EstimatedMinutes` と `Context` を補助的に見ながら**、残り時間で締切タスクや通常タスクを選定し、具体的な時間枠を割り当てる
8. **時刻付きのスケジュールを DailyNote に記載**
9. **`ScheduledDate` / `DueDate` を変更した場合は理由を DailyNote に記録**
...
ただし、DailyNote のタイムスケジュール表そのものは plain text だけでも読める必要がある。時刻付きスケジュール本体は plain text 優先で書き、`:entry{#...}` は補助参照として併記する。
...
- [ ] 週次レビューでは、`Organization::WeeklyReview = true` の各 Organization に今週解くべき課題をちょうど1つ設定したか

	...
	handle, pid = uv.spawn(textlint_bin, opts, function(code, signal)
		if code > 0 then
			vim.schedule(function()
				status_err("textlint init failed:", code, signal)
			end)
		else
			vim.schedule(function()
				status_msg("textlint init success")
			end)
		end
		uv.close(handle)
	end)
...
local function install_packages(...)
	...
	handle, pid = vim.loop.spawn(cmd, opts, function(code, signal)
		if code > 0 then
			vim.schedule(function()
				status_err("textlint add package failed:", code, signal)
			end)
		else
			vim.schedule(function()
				status_msg("textlint add package success:", package)
			end)
		end
		uv.close(handle)
	end)
